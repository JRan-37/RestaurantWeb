...
class menuAll extends Component {
    ...
    componentDidMount(){
        axios.get("http://localhost:8080/api/menu/categories").then((response)=>{
    ...
    render() {
        ...
        let menucategories = Object.keys(this.state.categories).map(id => <Single name={this.state.categories[id].category} key={this.state.categories[id].category} click={this.loadStateHandler} />);
